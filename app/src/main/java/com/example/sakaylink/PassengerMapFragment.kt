...
class PassengerMapFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        startButton = rootView.findViewById(R.id.request_ride_button)

        lifecycleScope.launch {
            val result = locationRepository.isUserAvailableOrVisible()
            if (result.isSuccess) {
                isAvailable = result.getOrNull() ?: false
        ...
        startButton.setOnClickListener {
            ...
            lifecycleScope.launch {
                locationRepository.updatePassengerVisibility(isAvailable)
            }
            startButton.backgroundTintList = ColorStateList.valueOf(
                ContextCompat.getColor(requireContext(), if (isAvailable) R.color.success_color else R.color.accent_color)
            )
        }
